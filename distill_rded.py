...
# if args.preload:
    ...
    for i in trange(len(dst_train)):
        sample = dst_train[i]  # 获取当前样本
        video_all.append(sample[0])  # 添加视频数据
        label_all.append(sample[1])  # 添加标签数据
        video_all_.append(sample[0])
        label_all_.append(sample[1])
        
        # 每处理完一个批次，进行拼接
        if len(video_all_) == batch_size or i == len(dst_train) - 1:
            # 拼接当前批次的所有视频数据和标签
            video_batch = torch.stack(video_all_)  # 拼接当前批次的所有视频
            label_batch = torch.tensor(label_all_)  # 拼接当前批次的标签

            # 将当前批次的数据添加到最终的列表中
            if len(video_all_) == batch_size:  # 如果是中间批次
                if len(video_all_) == batch_size:
                    video_all_ = []  # 清空批次数据
                    label_all_ = []  # 清空标签数据

            # 将当前批次数据加入最终结果（使用 `final_video` 和 `final_labels` 存储）
            if flag == 0:
                final_video = video_batch
                final_labels = label_batch
                flag = 1
            else:
                final_video = torch.cat((final_video, video_batch), dim=0)
                final_labels = torch.cat((final_labels, label_batch), dim=0)

    # 完成数据预加载，最后将其包装成 `TensorDataset`
    dst_train = torch.utils.data.TensorDataset(final_video, final_labels)
# print(66666666666)
# print(dst_train.size())
    ...
    project_name = "RDED_{}".format(args.method)
    ...
    # if args.preload else dst_train.labels
    ...
    image_syn = torch.stack(video_all)
    label_syn = torch.tensor(label_all)
    # if args.init == 'real':
    ...
    # if it in eval_it_pool and (save_this_best_ckpt or it % 1000 == 0):
    image_save = image_syn.detach()
    save_dir = os.path.join(args.save_path, project_name)
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    torch.save(image_save.cpu(), os.path.join(save_dir, "images_.pt"))
    if save_this_best_ckpt:
        save_this_best_ckpt = False
        torch.save(image_save.cpu(), os.path.join(save_dir, "images_best.pt"))
