...
def copy_and_rename_train_folders(source_folder, destination_folder, csv_file_path):
    """
    根据CSV文件中标记为'train'的行，将对应的文件夹从源文件夹复制到目标文件夹。
    ...
    # 读取CSV文件并获取'split'为'train'的folder_names
    train_folders = []
    with open(csv_file_path, 'r', newline='', encoding='utf-8') as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
            if row['split'].strip().lower() == 'train':
                train_folders.append(row['folder_name'].strip())
    
    print(f"找到 {len(train_folders)} 个 'train' 分割的文件夹。")
    ...
    # 使用 tqdm 显示处理进度
    pbar = tqdm(total=len(train_folders), desc="处理文件夹")
...
def main():
    ...
    示例：
    python script.py /mnt/nas-new/home/yangnianzu/cyf/video_distillation/distill_utils/data/UCF101/jpegs_112 /mnt/nas-new/home/yangnianzu/cyf/video_distillation/distill_utils/data/UCF101/train_jpeg /mnt/nas-new/home/yangnianzu/cyf/video_distillation/distill_utils/data/UCF101/labels.csv
    """
    if len(sys.argv) != 4:
    ...
    # 执行复制和重命名操作
    copy_and_rename_train_folders(source_folder, destination_folder, csv_file_path)
