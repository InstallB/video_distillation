...
import tqdm
from distill_utils.dataset import Kinetics400, UCF101, HMDB51, miniUCF101, staticHMDB51, staticUCF101, staticUCF50, singleSSv2, singleKinetics400,RDED
...
def get_dataset(dataset, data_path, num_workers=0,img_size=(112,112),split_num=1,split_id=0,split_mode='mean'):
    if dataset == 'MNIST':
        ...
        # print(333333333333333333)
        dst_train = RDED(path, split="train", transform=transform) # no augmentation
        dst_test  = RDED(path, split="test", transform=transform)
        print("RDED train: ", len(dst_train), "test: ", len(dst_test))
        class_names = None
    elif dataset == 'HMDB51':
        # this is a video dataset
        channel = 3
...
def get_loops(ipc, dataset=None):
    ...
    if ipc == 1 or ipc ==5 or ipc ==2 or ipc ==4 or ipc ==16:
